...
// i18n up the page
refreshes.i18n.populateElementsCopy();
...
function clearReFreshes() {
  host_list.innerHTML = host_list.innerHTML = '<li>' + refreshes.i18n.url_list_empty() + '</li>';
  refreshes.removeAll();
}

// Add the list of URLs/Hosts were tracking
host_list.innerHTML = '<li>' + refreshes.i18n.url_list_empty() + '</li>';
if (refreshes.reFreshes != undefined) {
  for (var i in refreshes.reFreshes) {
    host_list.innerHTML = '';
    break;
  }
  for (var i in refreshes.reFreshes) {
...
clear_all.addEventListener('click', clearReFreshes, false);
