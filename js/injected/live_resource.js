...
LiveResource.prototype.nonCacheURL = function() {
  if (this.url.indexOf('?') > 0) {
    return this.url + '&reFresh=' + (new Date() * 1);
  }
  return this.url + '?reFresh=' + (new Date() * 1);
}
...
LiveResource.prototype.check = function(callback) {
  ...
  this.xhr.ontimeout = this.xhr.onerror = function(){
    window.$reFresh.removeResource(_this.url);
  }
  ...
  this.xhr.onreadystatechange = function() {
    ...
    if (this.readyState == 4 && this.status != 304) {
      ...
      if (_this.checkResponse()) {
        setTimeout(function(){
          window.$reFresh.saveScrollPosition();
          _this.refresh();
        }, window.$reFresh.options.refresh_delay);
      }
    }
  }
...
LiveResource.prototype.refresh = function() {
  ...
  try {
    ...
    document.location.reload(window.$reFresh.url);
  }
}
...
LiveResource.prototype.tidyCode = function(html) {
  if (window.$reFresh.options.tidy_html == true) {
    ...
    // Remove references to ReFreshes cache breaker
    html = html.replace(/reFresh=([0-9]+)/gim, '');

    // Ignore doubleclick links
    html = html.replace(/"https:\/\/ad.doubleclick.net(.*?)"/gim, '');
  }

  if (window.$reFresh.options.ignore_hidden_fields === true) {
    // Remove script tags and hidden inputs
    html = html.replace(/<input(.*?)hidden(.*?)>/gim, "");
  }

  if (window.$reFresh.options.ignore_inline_js === true) {
    // Remove script tags and hidden inputs
