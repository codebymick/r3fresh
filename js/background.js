...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse){
  ...
  // If refresh is on this guy, lets start it.
  if (refreshes.isLive(tab.url)) {
    refreshes.start(tab);
  } else {
    refreshes.setEnableOnText(tab);
  }
});
...
chrome.browserAction.onClicked.addListener(function(tab) {
  ...
  if (refreshes.isLive(tab.url)) {
    refreshes.remove(tab);
  } else {
    // It's not live, so start R3Fresh on it.
    
    // If it's file:// check we have permission (file:// access enabled) to run R3Fresh on it.
